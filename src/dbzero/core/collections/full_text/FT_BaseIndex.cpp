...

{
    ...
    template <typename IndexKeyT>
    bool FT_BaseIndex<IndexKeyT>::addIterator(FT_IteratorFactory<UniqueAddress> &factory, IndexKeyT key) const
    {
        using ListT = typename super_t::ListT;
        auto inverted_list_ptr = this->tryGetExistingInvertedList(key);
        if (!inverted_list_ptr) {
    ...
    template <typename IndexKeyT>
    std::pair<IndexKeyT, UniqueAddress> FT_BaseIndex<IndexKeyT>::TagValueBuffer::const_iterator::operator*() const
    {
        if (m_values_it != m_values_end) {
            return *m_values_it;
        } else {
    ...
    FT_BaseIndex<IndexKeyT>::TagValueList::TagValueList(TagValueBuffer &&buf)
        : std::vector<std::pair<IndexKeyT, UniqueAddress> >(buf.m_values.begin(), buf.m_values.end())
    {
