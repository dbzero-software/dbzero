...

{
    ...
    MS_MetaPrefix::MS_MetaPrefix(std::size_t page_size, SparsePair &sparse_pair, const Diff_IO *diff_io_ptr)
        : MetaPrefix(page_size, sparse_pair)
        , m_ps_shift(db0::getPageShift(page_size))        
        , m_diff_io_ptr(diff_io_ptr)
    {
    }

    Allocator::SlotId MS_MetaPrefix::slotIdFromPageNum(std::uint64_t page_num)
    {
        return MS_Address::from(page_num << m_ps_shift).slot_id();
    }

    std::pair<std::uint64_t, std::uint64_t> MS_MetaPrefix::getPageRange(Allocator::SlotId slot_id)
    {
        ...
        return { first_addr >> m_page_shift, end_addr >> m_page_shift };
    }
    ...
    bool MS_MetaPrefix::evictSlot(Allocator::SlotId slot_id)
    {
        ...
        evictCleanPageRange(first_page_num, end_page_num);
        return true;
    }
    ...
    {
        if (!m_diff_io_ptr) {
            THROWF(db0::InternalException) << "MS_MetaPrefix: lazy slot loading requires Diff_IO reference";
        }
        auto [first_page_num, end_page_num] = MS_MetaPrefix::getPageRange(slot_id);
        ...
        db0::load(*this, *m_diff_io_ptr, slot_page_nums);
    }
