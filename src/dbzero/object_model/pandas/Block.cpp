...

{
    ...
    /*template <typename LangToolkit> o_block_item createBlockItem(const Block &block, 
    ...
    void Block::append(ObjectPtr lang_value)
    {
        ...
        v_bvector::push_back(
            createMember<LangToolkit>(*this, type_id, lang_value, m_storage_class) 
            );
    }
    ...
    void Block::setItem(std::size_t i, ObjectPtr lang_value)
    {
        if (i >= size()) {
            THROWF(db0::InputException) << "Index out of range: " << i;
        }
        ...
        v_bvector::setItem(i, createMember<LangToolkit>(*this, type_id, lang_value, storage_class));
    }
