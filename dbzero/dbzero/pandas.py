...
class BlockInterface(ExtensionArray):
    ...
    * interpolate"""
    ...
    def _compare(self, other, op) -> ArrayLike:
        """
        Helper method for comparison operations.
        
        Parameters
        ----------
        other : scalar or list
        ...
        """
        vals = [self._values[i] for i in range(len(self._values))]
        if not isinstance(other, list):
            other = [other]
        return np.array([op(a, b) for (a, b) in itertools.product(vals, other)])
    ...
    def __ne__(self, other: Any) -> ArrayLike:  # type: ignore[override]
        """
        Return for `self != other` (element-wise inequality).
        """
        return self._compare(other, lambda a, b: a != b)
