...
def test_memodata_class_default_args(db0_fixture):
...
def test_memodata_class_default_generated_args(db0_fixture): 
    obj_1 = MemoDataClassGenArgs(event_type = "Some Event")
    assert obj_1.event_type == "Some Event" 
    assert obj_1.event_name == "a_b"
    
    
def test_memodata_class_default_args_factory(db0_fixture):
